...
def simulate_iterative(slots, start_soc_pct=0.40, battery_kwh=18.08, dno_limit=4.0, max_charge_rate=5.5, max_discharge_rate=5.5, slot_hours=0.5):
    """
...
def simulate_msc(slots, start_soc_pct=0.40, battery_kwh=18.08, max_soc_pct=0.90, dno_limit=4.0, max_charge_rate=5.5, max_discharge_rate=5.5):
    """MSC baseline — 90% max."""
        ...
        results.append(
            {
                ...
                "soc_pct": soc / battery_kwh * 100,
                "target_soc": max_soc,
                "target_pct": max_soc / battery_kwh * 100,
                "export": export,
                "curtailed": curtailed,
                "charge": charge,
                "discharge": discharge,
...
def print_table(results, title, show_range=(4.5, 21.5)):
    ...
    for r in results:
        if show_range[0] <= r["time"] <= show_range[1]:
            ...
            print(f"{fmt(r['time']):>5} | {r['pv']:>5.2f} {r['load']:>5.2f} {r['excess']:>5.1f} | " f"{chrg} {disc} {r['soc_pct']:>5.1f} {r['target_pct']:>5.0f} {r['soc']:>6.2f} | " f"{r['export']:>5.2f} {crt}")
