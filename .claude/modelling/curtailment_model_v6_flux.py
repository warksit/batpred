        ...
        results.append(
            {
                ...
                "soc_pct": soc / BATTERY_KWH * 100,
                "target_soc": curtailment_target,
                "target_pct": curtailment_target / BATTERY_KWH * 100,
                "export": exp,
                "curtailed": curtailed,
                "charge": charge,
                "discharge": discharge,
...
def print_table(results, title, show_range=(4.5, 21.5)):
    ...
    for r in results:
        if show_range[0] <= r["time"] <= show_range[1]:
            ...
            print(f"{fmt(r['time']):>5} | {r['pv']:>5.2f} {r['load']:>5.2f} {r['excess']:>5.1f} | " f"{chrg} {disc} {r['soc_pct']:>5.1f} {r['target_pct']:>5.0f} {r['soc']:>6.2f} | " f"{r['export']:>5.2f} {crt} | {flux}")
