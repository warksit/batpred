...
4. **Max export**: max of MEAN export values — flag only if MEAN > 4.0 kW (sustained breach). Do NOT flag transient max-sample spikes — those are grid-balancing artifacts during PV/load transitions and only matter if they cause a SIG fault (covered separately).
...
### Performance
- Export: mean max X.X kW [✓ means within DNO / ⚠ sustained breach at HH:MM] — transient max-sample spikes ignored unless caused SIG fault
