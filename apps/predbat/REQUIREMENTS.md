...
| **Power Up vs Power Down** (and the session window shape) | `ha/octoplus_session_helpers.yaml` — state AND the `current/next_joined_event_*` attributes | `sig_dispatch_intent_helpers.yaml` · `sig_dispatch_heartbeat.yaml` · `sig_keep_floor_guard.yaml` · `octoplus_power_up_grid_charge.yaml` · `curtailment_plugin` — all **read** the sensor ✓, none re-derives it. HA gives attribute templates no shared scope, so the `!= 0` test genuinely repeats six times *inside that one file* | `test_window_attributes_agree_with_the_sensor_state` renders every copy and asserts agreement · `test_plugin_reads_only_attributes_this_file_publishes` · `test_no_consumer_reimplements_the_discriminator` |
