...
| **`_discharge_efficiency()`** — two callers that MUST NOT agree | `curtailment_plugin._discharge_efficiency` (one definition) | `compute_session_reserve` **divides** by it ✓ — converts a *grid-side* export cap into pack energy · `_refresh_overnight_target` must **NOT** divide (RD39) — `morning_gap_load` is *already* pack-side. Do not "fix" this asymmetry: the two callers start from different sides of the inverter | `test_R55_morning_gap_is_not_divided_by_discharge_efficiency` |
