...
| **Power Up vs Power Down** (and the session window shape) | `ha/octoplus_session_helpers.yaml` — the two binary sensors AND the three `sensor.octoplus_power_down_{minutes,start,end}` window sensors (sensors not attributes: the template config flow has no attributes field) | `sig_dispatch_intent_helpers.yaml` · `sig_dispatch_heartbeat.yaml` · `sig_keep_floor_guard.yaml` · `octoplus_power_up_grid_charge.yaml` · `curtailment_plugin` — all **read** the sensor ✓, none re-derives it. each template renders independently, so the `!= 0` test genuinely repeats five times *inside that one file* | `test_window_agrees_with_the_sensor_state` renders every copy and asserts agreement · `test_plugin_reads_only_window_sensors_this_file_publishes` · `test_no_consumer_reimplements_the_discriminator` |
| **Session sell floor (RD44)** | `curtailment_calc.session_sell_floor_kwh` = `max(drain_floor_pct, keep_floor_pct)` | `ha/sig_dispatch_intent_helpers.yaml` `sig_dispatch_kw` re-derives it in Jinja, because that is where the setpoint is computed and a template cannot call Python. Both must clamp at the same SOC or the card promises a stop the dispatcher will not make | `test_jinja_session_floor_matches_the_python_definition` — probes the DEPLOYED sensor either side of the Python floor; an earlier version rebuilt the formula inline and passed with the real template drifted from `max()` to `min()` |
| **`_discharge_efficiency()`** — two callers that MUST NOT agree | `curtailment_plugin._discharge_efficiency` (one definition) | `compute_session_reserve` **divides** by it ✓ — converts a *grid-side* export cap into pack energy · `estimate_session_end_kwh` **divides** the export term only ✓ (RD42) — same conversion, same direction; its `load`/`pv` terms must NOT be divided · `_refresh_overnight_target` must **NOT** divide (RD39) — `morning_gap_load` is *already* pack-side. Do not "fix" this asymmetry: the callers start from different sides of the inverter | `test_R55_morning_gap_is_not_divided_by_discharge_efficiency`, `test_session_end_soc_projection` |
...
| **Big-overflow advice shortfall** | `curtailment_plugin.publish()` → `advice_shortfall_kwh` on the phase sensor (2026-08-10) | `binary_sensor.big_overflow_shortfall` · `binary_sensor.big_overflow_shortfall_high` · `big_overflow_load_advice.yaml` message — all three now **read** it ✓. Previously each re-derived `overflow − (1 − soc/100) × 18.08` with the pack size hardcoded three times. Helper templates recorded in `ha/template_helpers_big_overflow.yaml` | `test_advice_shortfall_is_computed_once_not_three_times` |
| **Dawn reserve** (overnight import protection) | `curtailment_plugin._dawn_floor_kwh` (RD21) | `compute_drain_above` floor arm (CM's own drain) · `_r63_floor_kwh` (RD32) · `input_number.predbat_best_soc_min` via `_set_predbat_export_floor` (RD34, binds **Predbat's** export plan) — all three read the one function | `test_dawn_reserve_floors_predbat_export_plan`, `test_r63_floor_is_the_dawn_reserve_not_the_deep_floor` |
...
| RD14c | IN FORCE | Saving sessions driven by **planned** times, not a lagging publication (2026-07-28). **Entities changed 2026-08-17** — the requirement is unchanged, what implements it is not. Octopus deleted the legacy binary sensor AND calendar in integration v19.0.0, and both had carried Power Ups as well as Power Downs. Meaning now comes from `binary_sensor.octoplus_power_down_active` (`ha/octoplus_session_helpers.yaml`), computed from each joined event's own start/end against `now()` — so it *is* the planned time the requirement asked for, and the ~1 min publication lag it was avoiding no longer exists. The heartbeat still takes native start/end TRIGGERS from `calendar...octoplus_power_down` (edges, not meaning). `curtailment_plugin` reads the same sensor for both `_is_session_dispatching` and `_is_saving_session_active`, so the ~43 s divergence RD42 found between them is now impossible by construction. |
| RD1–RD20 | IN FORCE | v30/v32 DC-coupled control layer — see Part 1 sections at the end. **RD17/RD28 narrowed by RD45 (2026-08-18)**: the no_drain Charge arm still runs, but only while CM is on the wheel, and CM no longer takes the wheel on a day with no curtailment risk. |
...
| RD29–RD38 | IN FORCE | v34 (2026-08-11/12): R63 retired; risk verdict has four states; advice keyed on the actual day not p90; RD34 floors **Predbat's** export plan via `input_number.predbat_best_soc_min`; RD36 drain ends at the forecast PV-covers-load crossing; RD37 session reserve stands aside when PV will fill it; RD38 dawn reserve is an explicit 45-min error margin. |
| RD39 | IN FORCE | **No `discharge_efficiency` divide on the morning gap** (2026-08-12). `load_minutes_step` derives from `sensor.sigen_plant_daily_load_consumption`, a balance residual (pv + discharge + import − export) that is **already pack-side** — it carries the ~95 W parasitic and the conversion loss. Dividing again double-counted (+0.36 kWh, growing with the window). Guard: `test_R55_morning_gap_is_not_divided_by_discharge_efficiency`. |
...
| RD43 | IN FORCE | **The pre-PV drain depth is the forecast requirement, nothing else** (2026-08-17). R62 returned `min(legacy, floor_driven)` with `legacy = soc_keep + buffer_pct% × soc_max`. The `min()` was one-way — the forecast could only ever drain DEEPER — so on a day needing no headroom (`floor_driven` → `soc_max`) the static term bound and drained anyway. Overnight `best_soc_keep` is 0, making `legacy` a bare 10% of pack size: on 17 Aug it dumped 5.1 kWh pre-dawn and refilled 3.5 kWh from PV three hours later, an 8.6 kWh round trip against a realised 4.08 kWh requirement the starting SOC already covered twice. `legacy` removed; `overflow_floor` is the whole answer. `input_number.curtailment_pre_pv_buffer_pct` deleted from HA. Guards: `test_R62_pre_pv_target_no_overflow_means_no_drain`, `test_R62_pre_pv_target_moderate_day_drains_only_to_headroom`. | **Amended by RD47 (2026-08-24):** depth is still the forecast requirement, but bounded below by a minimum floor. That is not a return of `legacy` — `legacy` was a `min()` that could only drain HARDER, whereas RD47 is a `max()` that can only limit an over-drain.
| RD44 | IN FORCE | **A paid session sells to the overnight reserve, not to the deep-discharge floor** (2026-08-18). The plugin already PUBLISHED this intent — `sig_keep_floor_pct` carries `overnight_target_kwh` for a session dump (RD20) and `curtailment_plugin` says "dump, which still stops at the overnight reserve" — but nothing enforced it. `sig_keep_floor_guard` deliberately stands off a live session, leaving only the RD22 clamp at `sig_drain_floor_pct` (1.0%), so a session could sell the entire night. Live 2026-08-17 the 18:00 dump ran to 30.8% against a 38% reserve and stopped only because the window closed. The RD22 clamp floor is now `max(sig_drain_floor_pct, sig_keep_floor_pct)` while `binary_sensor.octoplus_power_down_active` is on — a `max()` so the deep floor still binds if higher, and the clamp can only ever sell LESS. It clamps the SETPOINT, not the policy, so there is no Max Export<->Hold flap at the floor: the battery stops discharging while PV surplus keeps exporting at the premium. **Pairs with the charge side:** the target is `overnight_target + session_reserve` deadlined to session START (`_minutes_to_session`, RD37 reachability), not to sundown — RD41 removed the session from `p10_recovery` for that reason. Under-charging therefore under-SELLS rather than emptying the night. **The card agrees with the dispatcher**: `estimate_session_end_kwh` clamps at the same floor (the clamp RD42 removed, legitimate now that something enforces it) and `estimate_session_export_left_kwh` is capped by the pack above it, so "still to sell" cannot exceed what can actually be sold. The floor has ONE definition, `session_sell_floor_kwh`, rendered against the Jinja by `test_jinja_session_floor_matches_the_python_definition`. Guards: `test_session_sell_stops_at_the_overnight_reserve`, `test_session_sell_runs_while_above_the_reserve`, `test_ordinary_drain_still_uses_the_deep_floor`, `test_session_floor_never_sells_below_the_deep_floor`, `test_session_floor_fails_safe_when_the_reserve_is_unreadable`, `test_session_end_projection_stops_at_the_enforced_floor`, `test_session_export_left_is_bounded_by_the_pack_above_the_floor`, `test_session_sell_floor_takes_the_higher_of_the_two_helpers`, `test_jinja_session_floor_matches_the_python_definition`. |
| RD45 | IN FORCE | **CM takes the wheel only when there is curtailment to manage** (2026-08-18, Andrew: "whenever overflow exists"). Active iff the safety-factored p90 does NOT fit the battery headroom (`compute_overflow_fits_margin` vs the early-handback buffer, same hysteresis as the `no_drain` veto). Deliberately WITHOUT the `peaked` requirement that gates `_overflow_fits_latched`: `peaked` stops the Hold/Drain gate calling the day too early, but the activation question — is any curtailment forecast to breach my headroom — is answerable from dawn, and requiring `peaked` would keep CM on the wheel every morning of every no-risk day. Live 2026-08-18: p90 0.69 kWh, verdict "fits", CM on the wheel from 06:40 charging to its own 58.7% session target while Predbat's plan banked 85% and sold the 18:00 session for ~60p more. **Applied at the take-the-wheel decision in `on_update`, NOT inside `calculate()`** — calculate still computes the floors and the policy CM *would* choose, so the observe-only sensor keeps publishing a real intention (the card renders it "shadowing only — CM would be X") and the R54/R55/safe_time/sundown paths still reach the maths they are tested on. An earlier cut gated inside `calculate()` and pre-empted three of those; the fixtures went green for the wrong reason. **Narrows RD17/RD28**, which kept CM active on no-overflow days to run the Charge arm for the evening reserve — that existed because CM was the only thing driving; the reserve is Predbat's job and Predbat plans it better. Agreed explicitly. **RD14-own outranks it**: never stands down during or minutes before a joined session, because the heartbeat only forces Max Export while CM holds the wheel and the select is not `Predbat`. Guards: `test_rd45_no_curtailment_risk_stands_down` (and the three `test_sundown_defers_*` session tests, verified failing with the guard removed). | **Narrowed by RD48/RD49 — both REMOVED by RD50 (2026-08-29); the stand-down is unconditional again.**
| RD46 | IN FORCE | **Predbat's charge plan is capped at the night's own need while CM is stood down** (2026-08-22). Predbat sizes its overnight reserve from its own median load forecast; CM's dawn drain defends the p90 Solcast headroom, and stock Predbat has no reference to CM anywhere — a one-way information asymmetry, not a bug in either optimiser. On the nights the two disagree Predbat buys a reserve CM then drains and exports: 2026-08-20 it planned `FrzChrg` 22:00-00:00 plus a 04:00 top-up holding ~45% into a morning CM took to ~1%, bought at 12.42p and sold at ~12p (Andrew overrode the slots to Demand by hand). CM now writes `input_number.predbat_best_soc_max` = its **live** `overnight_target_kwh` via `_set_predbat_charge_cap`, the charge-side mirror of RD34's `_set_predbat_export_floor`. **The overnight target, not a low fixed %**: it self-liquidates (buy what the night burns and it is gone by dawn, so CM still gets its headroom) while leaving Predbat free to buy cheap energy for the house's own load — a 10% cap would win under 1 kWh of headroom out of ~18 and cost ~55p on a winter night. **Live, not static**: `compute_morning_gap` measures from now to sunrise, so the cap shrinks through the night (6.76 kWh at 22:00, ~2 kWh by 04:00) — that is what kills the pre-dawn top-up. **Cleared (0) on any of three conditions**, decided in `_predbat_charge_cap_kwh` from the FINAL value of `_cm_controlling` rather than inside any one branch: CM driving; the plugin disabled; or **the dawn crossing having passed**. The dawn gate is load-bearing and is NOT merely belt-and-braces: `charge_limit` maps to `number.sigen_plant_ess_charge_cut_off_state_of_charge` (apps.yaml), the SOC at which the pack stops charging from ANY source **including solar** (the charge_limit=0 solar-blocking bug of 2026-03-17). the cap only reaches it inside a charge window. **CORRECTION 2026-08-26:** this row originally said Predbat holds that register at 100% outside charge windows via `inverter_soc_reset` — **it does not.** All four reset-to-100% branches are gated on `not self.inverter_hybrid` (`execute.py:509, 545, 566, 578`) and live `inverter_hybrid` is **on**, so they are dead code here and `inverter_soc_reset` is inert. Outside a charge window Predbat in fact commands the register to **1%** (`set_reserve_min`), and most such writes do not reach the plant only because the verify-read is a no-op behind an HA key (`ha.py:763`). The dawn gate remains correct, but for this reason rather than the one first recorded — but a daytime charge window (an Octoplus Power Up, say) with the cap still set would write a low cut-off and block solar exactly when the pack should be absorbing. "Clear when CM drives" does not cover it: post-RD45 CM never takes the wheel at all on a low-overflow day, so nothing would clear it; `_dawn_released` is the one signal that fires on every day-type, and is the same one-way daily latch RD34 uses. **Does NOT suppress a charge freeze** — `best_soc_max` clamps `loop_soc`, which seeds the Chrg/HoldChrg candidates, while the freeze candidate is `self.reserve` appended independently of it (`plan.py:1508-1519`). The residual ~0.56 kWh of incidental import (a freeze blocks discharge, so load falls through to the meter; it never actively imports to raise SOC) is accepted: it is neutral in Predbat's own model, since leftover battery is valued at `rate_min_forward` = the same 12.42p as the cheap window. **Ruled out, do not revisit without new evidence**: toggling `switch.predbat_set_charge_freeze` (carries `reset_inverter: true`, so every toggle forces a full inverter reset) and lowering `metric_battery_cycle` (freezing is exactly neutral at any penalty). Guards: `test_rd46_night_cap_is_the_live_overnight_target`, `test_rd46_cap_tracks_the_target_down_through_the_night`, `test_rd46_cap_cleared_when_cm_takes_the_wheel`, `test_rd46_cap_cleared_under_a_manual_override_that_drives_cm`, `test_rd46_cap_cleared_once_pv_meets_load`, `test_rd46_cap_not_rewritten_when_unchanged`, `test_rd46_no_cap_written_while_the_target_is_unknown`, `test_rd46_charge_limit_maps_to_the_solar_blocking_register`, `test_rd46_best_soc_max_caps_charge_candidates_but_not_the_freeze`. |
| RD47 | IN FORCE | **The overflow floor grades instead of saturating at zero** (2026-08-24, Andrew: "we maybe don't need to defend so aggressively (to 1%)… something more gradual?"). `required_headroom_kwh` applies a 1.05x multiplier **and** a flat 1.8 kWh reserve, so on an 18.08 kWh pack the floor hits 0 at only **15.50 kWh** of forecast overflow and passes 10% at 13.78 — **the whole 10%->0% gradation is 1.7 kWh wide**, above which a 16 kWh and a 30 kWh forecast are indistinguishable. Live `sensor.predbat_curtailment_floor_overflow` read **0.00 all morning on both 2026-08-23 and 08-24**. On 08-23 that emptied the pack to 1.0% on a day that realised **12.81 kWh** and peaked at 71.6% — 28 points never used. **The aggression was the buffers, not the forecast:** at 15.5 kWh they add 2.6 kWh on top of the raw number, which is what tips an otherwise-fitting day to zero. `soften_overflow_floor` now holds a minimum floor (`input_number.curtailment_min_floor_pct`, default **10%**) until the RAW forecast exceeds what that minimum leaves (`soc_max - min_floor` = 16.27 kWh), then tapers to zero by `soc_max` — both bounds **derived**, no magic constants. Applied at **exactly two sites**, `curtailment_plugin` live daytime and `compute_pre_pv_target` (the dawn path that actually empties the pack); `required_headroom_kwh` is deliberately untouched so fits/R48/RD28 do not move. **Direction matters:** it is a `max` against a minimum, so it can only ever limit an over-drain — where RD43 removed a `min` that could only deepen one. **Evidence:** 26 days of `sensor.curtailment_overflow_daily` (from 2026-07-29) — median ~9 kWh, max 17.04 — realised overflow exceeded 16.27 kWh **once in 26 days** (short by 0.77 kWh ~ 12p). That is the accepted price, weighed against R25/R11's asymmetry, which is also why the taper still reaches zero rather than holding a reserve on a day the pack cannot cover regardless. **Confidence deliberately NOT used:** it points the wrong way (high confidence => P90~P50, so defending P90 is nearly free; low confidence => P90>>P50, exactly when a "relax unless confident" rule under-prepares — R50a's 27 Jul fixture, blend 2.35 kWh vs ~13 kWh absorbed), and it could not have fixed 08-23 (confidence 0.90-0.93). **R50a stays retired; `_expected_overflow` is untouched.** Guards: `test_rd47_floor_no_longer_saturates_at_zero`, `test_rd47_holds_the_minimum_until_the_forecast_needs_it`, `test_rd47_aug23_replays_to_the_minimum_not_zero`, `test_rd47_extreme_forecast_still_empties_the_pack`, `test_rd47_never_raises_the_floor_above_the_forecast_requirement`, `test_rd47_minimum_of_zero_disables_the_softening`, `test_rd47_minimum_is_clamped_so_the_dashboard_cannot_invert_it`, `test_rd47_pre_pv_target_grades_too`, `test_rd47_both_call_sites_agree`. |
| RD48 | ❌ **REMOVED by RD50 (2026-08-29)** | **CM keeps the wheel while banking would displace exportable PV** (2026-08-24). CM reaching outside its brief to patch a Predbat behaviour, on a gate that does not say what it means: the "useful ceiling" is at its daily MINIMUM at dawn, so it latched on days with zero forecast overflow and held the wheel to sundown. Original row and evidence in Part 2 (History). **Do not cite as authority.** |
| RD49 | ❌ **REMOVED by RD50 (2026-08-29)** | **CM holds the wheel for the whole solar day once the pack is past the useful ceiling** (2026-08-26). CM reaching outside its brief to patch a Predbat behaviour, on a gate that does not say what it means: the "useful ceiling" is at its daily MINIMUM at dawn, so it latched on days with zero forecast overflow and held the wheel to sundown. Original row and evidence in Part 2 (History). **Do not cite as authority.** |
| RD50 | IN FORCE | **No overflow, no CM — without exception** (2026-08-29, Andrew: "if no overflow cm should not run. Or it will break winter"). RD45 said CM takes the wheel only when there is curtailment to manage; RD48/RD49 then carved out an exception for a Predbat behaviour (banking surplus it could have exported) and the exception ate the rule. **The gate did not say what it meant:** the "useful ceiling" is `overnight need + max(session need, cap x 1 h)`, and the overnight need has just been SPENT at dawn — live 2026-08-29 07:00 it read 0.22 kWh, putting the ceiling at 3.90 kWh against a 46.2% pack (8.35 kWh of 18.08). CM latched at **06:40:37** with PV at 0.303 kW and p10/p50/p90 overflow all **0.00**, set `read_only` (`predbat.status` Read-Only 06:41:25), and by RD49's solar-day rule would have held every lever to sundown. **Winter is that day repeated:** overflow is zero for months while the pack routinely sits above one export hour, so the exception would have CM driving all season on a curtailment brief with no curtailment — the failure RD45 exists to end. **Predbat misbehaviour is fixed upstream, not sat on from outside** (Andrew: "If predbat misbehaves to what I want I will have to sort that with them"). The only exception that survives is **RD14-own**: a live or imminent joined session keeps CM driving, because the heartbeat only forces Max Export while CM holds the wheel. Guards: `test_rd50_stands_down_on_a_no_risk_day_however_full_the_pack` and `test_rd50_stands_down_at_dawn_on_a_no_risk_day` (both verified failing against RD48/RD49's code; the second replays 08-29 dawn), `test_rd50_soc_cannot_retake_the_wheel_on_a_no_risk_day` (also verified failing), `test_rd50_stands_down_below_the_old_ceiling_too`, `test_rd50_takes_the_wheel_when_overflow_does_not_fit` (the rule has two halves — without it the suite would pass on a CM that never drives at all), `test_rd50_session_still_outranks`. |
...
- **R52** (v22 pre-PV drain timing): activate the plugin BEFORE sunrise on
  ...
  is uncontested by PV. Two-stage drain:
    - Pre-PV: target = R62 `overflow_floor` (RD43 — was `soc_keep + buffer_pct
      × soc_max`, removed 2026-08-17)
...
```text
overflow_floor = max(0, (soc_max − min(1.8, overflow)) − overflow × 1.05)
target         = max(reserve,
                     DEEP_DISCHARGE_FLOOR + dawn_load,
                     overflow_floor)
```

- `overflow` = overflow integral against the R60 effective cap (already
  computed pre-dawn by _publish_forecast_overflow). A throttled grid (smaller
  effective cap) raises it and so automatically deepens the drain.
...
- A small overflow leaves `overflow_floor` near `soc_max`, so the target sits
  at or above current SOC and the caller drains **nothing**. That is the
  intended answer on a day that does not need headroom.
- Implemented in `compute_pre_pv_target()` (curtailment_calc.py).

**RD43 (2026-08-17) — the legacy term is REMOVED.** As first written this was
`target = min(legacy, floor_driven)` with `legacy = soc_keep + buffer_pct% ×
soc_max`, described above as a knob the forecast could only override by being
"MORE aggressive, never less". A `min()` in that position is one-way: the
forecast could deepen the drain and never soften it, so whenever the forecast
said *no headroom needed* (`floor_driven` → `soc_max`) the static term bound
and drained anyway.

On 17 Aug 2026 that dumped 5.1 kWh pre-dawn (51.5% → 21.4% SOC) and refilled
3.5 kWh from PV three hours later — an 8.6 kWh round trip on a day whose
realised headroom requirement was 4.08 kWh, which the SOC it started at already
covered twice over. Overnight `best_soc_keep` was **0**, so `legacy` was a bare
10% of pack size: `drain_above` published exactly 1.81 (= 0 + 0.10 × 18.08),
which is how we know the static term was binding and `floor_driven` wanted
little or no drain.

The term cannot be rescued by retuning the helper. With `soc_keep` at 0
overnight it is a percentage of pack size carrying no information about the
...
## RD48 / RD49 — the export-surplus hold (2026-08-24 to 2026-08-29) — HISTORICAL

> **❌ REMOVED by RD50 on 2026-08-29, four days after RD49 landed.** Kept for the
> reasoning. Both rows are reproduced verbatim below; neither is in force.
>
> **Why they were removed.** Both were built on a real observation — Predbat, given
> the day, banks PV surplus into the pack while the export cap sits idle — and both
> fixed it from the wrong side of the boundary. CM's brief is to minimise
> curtailment; on a day with no forecast overflow there is nothing for it to
> minimise, and RD45 had already settled that it stands down. RD48 carved out an
> exception, RD49 widened it by deleting the `surplus > cap` half of the test, and
> what was left — "SOC above overnight-need + one export hour" — is true on most
> days of the year. The ceiling is at its daily MINIMUM at dawn, because the
> overnight need has just been spent: live 2026-08-29 it was 3.90 kWh, so a 46.2%
> pack cleared it at 06:40 with 0.303 kW of PV and zero forecast overflow, and
> RD49's solar-day rule then held the wheel until sundown. Through the winter,
> when overflow is zero for months and the pack is nearly always above one export
> hour, that is CM driving every day of the season for a job it does not have.
>
> **The general lesson**, which is why this section exists rather than a one-line
> removal: an exception written to catch one live afternoon was gated on a
> quantity that happens to be small then and is small nearly always. Before adding
> an exception to a stand-down rule, evaluate its gate on the day-types it was NOT
> written for — dawn, midwinter, an empty forecast — not only on the day that
> prompted it. Predbat behaving in a way this site does not want is an upstream
> conversation, not a reason for CM to take the wheel.

**RD48, as it stood:**

| RD48 | (was IN FORCE, now REMOVED) | **CM keeps the wheel while banking would displace exportable PV** (2026-08-24, Andrew: "CM should have stayed exporting all spare PV rather than importing to battery to export tonight/tomorrow"). Live 16:40 with CM stood down and Predbat driving: PV 5.623 kW, load 0.370, **surplus 5.25 kW going entirely into the battery**, grid export **−0.005 kW** with the 3.68 kW DNO cap idle, SOC 75.9% and climbing. **Useful fill is only ~59% of pack** — 36.1% overnight target plus the ~21% a high-rate export window can physically absorb (`cap × 1 h` = 3.68 kWh = 20.4%, Andrew's "38 plus 21 for session") — so ~7.5 kWh was being banked with no buyer: it cannot be sold at the peak (only 21% fits) and is not needed overnight. **The cost is curtailment, not the round trip:** filling at 5.2 kW fills the pack ~17:30 while surplus still exceeds the cap, so the remainder is curtailed; exporting at the cap and banking only the above-cap ~1.57 kW fills it ~19:25 after PV has fallen, curtailing nothing (R25 — after overflow there are no levers). `_export_hold_active` keeps `phase == "active"` with `_policy_override = "hold"` while **both** the pack is above `_useful_ceiling_kwh` **and** surplus exceeds the export cap. **Hold, never Max Export** — selling the pack at the 12p daytime rate is worse than keeping it to displace ~25p overnight import; RD48 stops banking, it does not start selling. **The second condition is what makes it safe:** holding the wheel sets `read_only`, which muzzles Predbat, so an unbounded hold would block Predbat's own high-rate window (20.5p at 18:00-19:00 vs 12.0p that afternoon). Tying it to surplus > cap is self-limiting — as PV falls there is nothing left to displace, CM hands back, and Predbat sells. It also keeps CM's trigger **physical, not price-aware**. **Re-taking is automatic** (the gate is re-evaluated every cycle), which is exactly why release is a **confirm count** (`EXPORT_HOLD_RELEASE_CYCLES` = 2), not a margin. The first cut released on `surplus > cap − 0.3` and flapped live within minutes of deploy — **Hold 17:19 → Predbat 17:25 → Hold 17:30**, banking surplus again for the cycle Predbat held it — because PV fell **4.288 → 1.747 kW in three minutes** behind one cloud, a swing no plausible margin absorbs. Engage still needs a clear margin (`EXPORT_HOLD_MARGIN_KW`) so a spike cannot grab the wheel; release now needs sustained absence, the same shape as `DAWN_RELEASE_CONFIRM_CYCLES`. Every swap toggles three mapper automations and `read_only` (2026-08-03: 7 swaps in 25 min). The original flap test hovered ±0.1 kW either side of the cap and passed while the real thing oscillated — replaced by `test_rd48_survives_a_single_cloudy_cycle`, verified failing against the margin version. Fails CLOSED on an unreadable sensor or an overnight target not yet computed. **Narrows RD45**, which stood CM down on any no-risk day on the reasoning that "the reserve is Predbat's job and Predbat plans it better" — true in general, and false in this one case; RD45 remains in force for the common low-SOC case. Guards: `test_rd48_keeps_the_wheel_while_banking_would_displace_export`, `test_rd48_rd45_still_stands_down_below_the_ceiling`, `test_rd48_hands_back_once_surplus_drops_below_the_cap`, `test_rd48_hands_back_once_soc_reaches_the_ceiling`, `test_rd48_ceiling_is_derived_not_hardcoded`, `test_rd48_never_max_export_on_this_path`, `test_rd48_can_retake_the_wheel_after_handing_back`, `test_rd48_survives_a_single_cloudy_cycle`, `test_rd48_releases_after_a_sustained_drop`, `test_rd48_session_still_outranks`. | **Superseded in part by RD49 (2026-08-26):** the `surplus > cap` condition is removed; the ceiling test and the Hold policy remain.

**RD49, as it stood:**

| RD49 | (was IN FORCE, now REMOVED) | **CM holds the wheel for the whole solar day once the pack is past the useful ceiling** (2026-08-26, Andrew: "rather than a pr to predbat, we should move the handback to predbat to end of solar"). RD48 held only while PV surplus exceeded the export cap, on the theory that below it everything exports anyway. **It does not — Predbat banks it.** Live 2026-08-26 15:06 with Predbat driving: SOC **61.4%** against a **61.2%** ceiling (overnight 7.18 kWh / 39.7% + session need 3.89 kWh / 21.5%), **+1.07 kW into the pack and 0.037 kW exported**, surplus ~1.05 kW — well under the 3.68 kW cap, so RD48's second condition never fired. Being past the ceiling is the whole test; the surplus never was. **Why this is safe now and was not when RD45 was written:** export is **flat at 12p** going forward (the 20.5p on 08-24 was a Power Down session, not a tariff peak), so Predbat has no price edge inside the solar day at all; and the one thing it did better on 2026-08-18 — sizing the bank for the session — CM now computes itself (RD41 `session_charge_target`). The ceiling accordingly uses the **real session need** when a session is joined, falling back to `cap × SESSION_ALLOWANCE_HOURS` otherwise. **The solar-day bound comes free:** `calculate()` returns "off" at sundown (the O1 latch), so `phase == "active"` stops holding and the caller drops the latch. `past_safe` deliberately does NOT end the day here — it only forces the Hold override — so CM keeps the wheel through the evening export window, which is the point. **Also narrows RD6**, which moved handback earlier (safe_time, not sundown) to stop CM's evening **drain** overreach (R56). Survivable because the policy here is **Hold**, never Max Export — holding does not sell the pack at the low daytime rate, which is what made R56 wrong. Release inside the day is only the pack dropping back under the ceiling, still on `EXPORT_HOLD_RELEASE_CYCLES` (SOC wobbles at the line); the surplus margin `EXPORT_HOLD_MARGIN_KW` is **removed**. **Unverified risk:** RD41's session charge target has never been checked with CM actually driving through a session — 2026-08-18 (CM banking to 58.7% while Predbat's plan would have sold ~60p more) is RD45's whole justification and the failure this could repeat. Watch the next session night. Guards: `test_rd49_holds_even_when_surplus_is_below_the_cap`, `test_rd49_releases_only_when_soc_falls_back_under_the_ceiling` (both verified failing against RD48's logic), plus the retained RD48 suite. |
